...
export default function ZonasH ()  {
    ...
    const saveProduct = () => {
        ...
        const newCountry ={
            timeZoneName: ciudad,
            utcNumberHours: rango,
            abbreviationCode: abreviacion,
            enabled: true
          };
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Zona horaria Creada', life: 3000 });
                setTimeout(() => {
                        setProductDialog(false);
                        setCargar(false)
                        try {
        ...
        const newCountry ={
            ...
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Zona Horaria Editada', life: 3000 });
                setTimeout(() => {
                        setProductDialog(false);
                        setLoading(true)
                        setCargar(false)
    ...
    const deleteProduct = () => {
        const newCountry ={
            timeZoneId: paisId,
            timeZoneName: "hardcode",
            utcNumberHours: "hardcode",
            abbreviationCode: "hardcode",
            enabled: true
          };
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Zona Horaria Eliminada', life: 3000 });
                setTimeout(() => {
                    setCargar(false)
                    setDeleteProductDialog(false);
                        try {
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="Nueva Zona Horaria" icon="pi pi-plus"  onClick={openNew} style={{backgroundColor:'#202c52', color:'white',borderColor: 'rgba(0,0,0,0)'}}/>
    ...
    const header = (
        <div className="table-header">
            <h5 className="mx-0 my-1">Administrar Zonas Horarias</h5>
        ...
        <div className="datatable-crud-demo">
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={datas} selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)} loading={loading}
                    dataKey="timeZoneId" paginator rows={10} rowsPerPageOptions={[5, 10, 25]}
                    paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                    currentPageReportTemplate="Mostrar {first} de {totalRecords} zonas"
                    globalFilter={globalFilter} header={header} responsiveLayout="scroll">
                    <Column selectionMode="single" headerStyle={{ width: '3rem' }} exportable={false}></Column>
                    <Column body={actionBodyTemplate}  header="Acciones" exportable={false} style={{ minWidth: '2rem' }}></Column>
                    <Column field="timeZoneName" header="Nombre de la zona" filter filterPlaceholder="Buscar por nombre" filterMatchMode="contains" sortable style={{ minWidth: '12rem' }}></Column>
                    <Column field="abbreviationCode" header="Abreviacion" filter filterPlaceholder="Buscar por abreviacion" filterMatchMode="contains" sortable style={{ minWidth: '12rem' }}></Column>
                    <Column field="utcNumberHours" header="Numbero de horas" filter filterPlaceholder="Buscar por numero" filterMatchMode="contains" sortable style={{ minWidth: '12rem' }}></Column>
                </DataTable>
            </div>
            ...
            <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirmación" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="confirmation-content">
                    ...
                    {product && <span> Deseas eliminar la siguiente zona <b>{product.timeZoneName}</b>?</span>}
                </div>
            </Dialog>
