...
export default function Impuestos ()  {
    ...
    const saveProduct = () => {
        setSubmitted(true);
        const newCountry ={
            taxTypeDescription: ciudad,
            taxTypeName: abreviacion,
            enabled: true
          };
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Impuesto Creado', life: 3000 });
                setTimeout(() => {
                        setProductDialog(false);
                        setCargar(false)
                        try {
        ...
        const newCountry ={
            ...
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Impuesto Editado', life: 3000 });
                setTimeout(() => {
                        setProductDialog(false);
                        setLoading(true)
                        setCargar(false)
        ...
        const newCountry ={
            ...
            try{
              ...
              .then((res)=>{
                if(res.codigo == 200){
                    setCargar(true)
                    toast.current.show({ severity: 'success', summary: 'Satisfactorio', detail: 'Impuesto Eliminado', life: 3000 });
                setTimeout(() => {
                    setCargar(false)
                    setDeleteProductDialog(false);
                        try {
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button label="Nuevo Impuesto" icon="pi pi-plus"  onClick={openNew} style={{backgroundColor:'#202c52', color:'white',borderColor: 'rgba(0,0,0,0)'}}/>
    ...
    const header = (
        <div className="table-header">
            <h5 className="mx-0 my-1">Administrar Impuestos</h5>
        ...
        <div className="datatable-crud-demo">
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={datas} selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)} loading={loading}
                    dataKey="taxTypeId" paginator rows={10} rowsPerPageOptions={[5, 10, 25]}
                    ...
                    <Column field="taxTypeName" header="Nombre de impuesto" filter filterPlaceholder="Buscar por impuesto" filterMatchMode="contains" sortable style={{ minWidth: '12rem' }}></Column>
                    <Column field="taxTypeDescription" header="Descripcion de impuesto" filter filterPlaceholder="Buscar por descripcion" filterMatchMode="contains" sortable style={{ minWidth: '12rem' }}></Column>
                </DataTable>
            </div>
            ...
            <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirmación" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="confirmation-content">
                    ...
                    {product && <span> Deseas eliminar el siguiente impuesto <b>{product.taxTypeName}</b>?</span>}
                </div>
            </Dialog>
